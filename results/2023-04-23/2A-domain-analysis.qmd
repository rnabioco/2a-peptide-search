...
# calculate the number of annoated domains for each record
...
```{r}
filter(domains_tbl, n_dom == 1) |> count(db)
```

Let's remove the singletons for now, will clean up interpretations later.

```{r}
domains_tbl <- filter(domains_tbl, n_dom > 1)
```
...
This is the distribution of numbers of domains per protein. The spike at 12 is from the large number of viral polyproteins with similar / identical structures.
...
ggplot(
  ...
  labs(
    x = "number of domains",
    y = "count",
    title = "Domains per protein",
    subtitle = glue("total records = {scales::comma(total_n)} (plot excludes counts < 5)")
  )
```
